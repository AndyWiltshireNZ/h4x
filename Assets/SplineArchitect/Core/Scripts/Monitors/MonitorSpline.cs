...
namespace SplineArchitect.Monitor
{
    public class MonitorSpline
    {
        ...
        private float[] floatContainer = new float[7];
        ...
        private void GetControlPointSums(Vector3[] sums)
        {
            ...
            foreach (Segment s in spline.segments)
            {
                ...
                if (s.ignoreSnapping) sums[0].x += 1;
            }
        }

        private void GetExtraSums(float[] sums)
        {
            sums[0] = 0; sums[1] = 0; sums[2] = 0; sums[3] = 0; sums[4] = 0; sums[5] = 0; sums[6] = 0;
        ...
        public bool SegmentChange(bool forceUpdate = false)
        {
            bool foundChange = false;
            Vector3 anchorPos = spline.segments[0].GetPosition(Segment.ControlHandle.ANCHOR);
            GetControlPointSums(Vector3Container);
            if(!GeneralUtility.IsEqual(anchorPos, anchor0, 0.001f)) foundChange = true;
            else if (!GeneralUtility.IsEqual(Vector3Container[0], achorSum, 0.001f)) foundChange = true;
            else if (!GeneralUtility.IsEqual(Vector3Container[1], tangentASum, 0.001f)) foundChange = true;
            else if (!GeneralUtility.IsEqual(Vector3Container[2], tangentBSum, 0.001f)) foundChange = true;
        ...
        public bool ExtraChange(bool forceUpdate = false)
        {
            ...
            GetExtraSums(floatContainer);

            if (!GeneralUtility.IsEqual(floatContainer[0], zRotationSum, 0.001f)) foundChange = true;
            else if (!GeneralUtility.IsEqual(floatContainer[1], contrastSum, 0.001f)) foundChange = true;
            else if (!GeneralUtility.IsEqual(floatContainer[2], saddleSkewXSum, 0.001f)) foundChange = true;
            else if (!GeneralUtility.IsEqual(floatContainer[3], saddleSkewYSum, 0.001f)) foundChange = true;
            else if (!GeneralUtility.IsEqual(floatContainer[4], scaleXSum, 0.001f)) foundChange = true;
            else if (!GeneralUtility.IsEqual(floatContainer[5], scaleYSum, 0.001f)) foundChange = true;
        ...
        public bool NoiseChange(bool forceUpdate = false)
        {
            ...
            if (!GeneralUtility.IsEqual(sum, noiseSum, 0.001f)) foundChange = true;
        ...
        public void ForceUpdateSplineObjects()
        {
