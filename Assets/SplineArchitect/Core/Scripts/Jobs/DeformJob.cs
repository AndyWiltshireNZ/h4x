...
namespace SplineArchitect.Jobs
{
    ...
    public struct DeformJob : IJobParallelFor
    {
        ...
        [ReadOnly] public NativeArray<SplineObject.NormalType> soNormalTypeMap;
        ...
        public void Execute(int i)
        {
            ...
            SplineObject.NormalType soNormalType = SplineObject.NormalType.SPLINE_SPACE;
            if (deformationType == SplineObject.Type.FOLLOWER)
            {
                localSpace = localSpaces[verticesMap[i]];

                if (alignToEndMap.Length == verticesMap.Length)
            ...
            else
            {
                ...
                for (int i2 = 0; i2 < verticesMap.Length; i2++)
                {
                    if (i < verticesMap[i2])
                    {
                        ...
                        if (soNormalTypeMap.Length == verticesMap.Length)
            ...
            //Normal
            if (meshNormals.Length == vertices.Length)
            {
                Vector3 forwardDir = normalType == Spline.NormalType.DYNAMIC ? zDirection : -zDirection;

                if (soNormalType == SplineObject.NormalType.SPLINE_SPACE)
                {
                ...
                else if (soNormalType == SplineObject.NormalType.CYLINDER_BASED)
                {
                ...
                //Tangent
                if (meshTangents.Length == vertices.Length)
                {
