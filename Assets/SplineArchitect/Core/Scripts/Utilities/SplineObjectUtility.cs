...
namespace SplineArchitect.Utility
{
    public class SplineObjectUtility
    {
        ...
        public static Bounds GetSplineOriginatedBounds(SplineObject so)
        {
            if (so.type == SplineObject.Type.DEFORMATION)
            {
                Mesh originMesh = so.meshContainers[0].GetOriginMesh();
                return GeneralUtility.TransformBounds(originMesh.bounds, GetCombinedParentMatrixs(so, true));
            }
            else if (so.type == SplineObject.Type.FOLLOWER)
            {
                ...
                if (meshFilter == null)
                {
                    ...
                    if (meshCollider != null && meshCollider.sharedMesh != null)
                        return GeneralUtility.TransformBounds(meshCollider.sharedMesh.bounds, GetCombinedParentMatrixs(so, true));
                }
                else
                {
                    if (meshFilter.sharedMesh != null)
                        return GeneralUtility.TransformBounds(meshFilter.sharedMesh.bounds, GetCombinedParentMatrixs(so, true));
                }
            }

            Debug.LogError($"[Spline Architect] Could not find valid bounds for {so.name}!");
            return new Bounds();
        }
    }
}