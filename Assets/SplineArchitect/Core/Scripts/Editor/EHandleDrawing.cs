    ...
    public static class EHandleDrawing
    {
        ...
        public static void OnSceneGUIGlobal(HashSet<Spline> splines, Event e)
        {
            ...
            foreach (Spline spline in allSelectedSplines)
            {
                ...
                if (EGlobalSettings.GetGridVisibility())
                    EHandleGrid.DrawGrid(spline.transform, EHandleGrid.GetGridBounds(spline), EGlobalSettings.GetGridSize(), spline.normalType == Spline.NormalType.STATIC_2D);
            }
        ...
        private static void DrawSpline(Spline spline, bool primiarySelection, bool secondarySelection, SplineHideMode hideMode)
        {
            ...
            for (int i = 1; i < spline.segments.Count; i++)
            {
                ...
                if ((primiarySelection || secondarySelection) && hideMode != SplineHideMode.SELECTED_OCCLUDED)
                {
        ...
        private static void DrawLinkDots(HashSet<Spline> splines, SplineHideMode hideMode, SceneView scneneView)
        {
            ...
            if (selectedSpline != null)
            {
                ...
                if(selectedSpline.selectedControlPoint != 0 && SplineUtility.GetControlPointType(selectedSpline.selectedControlPoint) == Segment.ControlHandle.ANCHOR)
                {
                    int selectedSegmentId = SplineUtility.ControlPointIdToSegmentIndex(selectedSpline.selectedControlPoint);
                    selectedAnchorPos = selectedSpline.segments[selectedSegmentId].GetPosition(Segment.ControlHandle.ANCHOR);
                }
            }
            ...
            if(EGlobalSettings.GetShowLinkIndicators())
            {
                foreach (Spline spline in splines)
                {
                    ...
                    {
                        if (s.linkTarget != Segment.LinkTarget.NONE && !segmentContainer.Contains(s))
                            segmentContainer.Add(s);
                    }
                }
            }
            ...
            foreach (Segment segment in segmentContainer)
            {
                ...
                Vector3 anchorPos = segment.GetPosition(Segment.ControlHandle.ANCHOR);
                ...
                if (segment.linkTarget == Segment.LinkTarget.SPLINE_CONNECTOR && segment.splineConnector != null)
                {
                ...
                else if(segment.linkTarget == Segment.LinkTarget.ANCHOR)
                {
