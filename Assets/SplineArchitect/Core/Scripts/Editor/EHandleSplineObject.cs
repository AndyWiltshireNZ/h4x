    ...
    public static class EHandleSplineObject
    {
        ...
        public static void OnSceneGUI(Spline spline, Event e)
        {
            ...
            if (e.type == EventType.Layout)
            {
                for (int i = spline.allSplineObjects.Count - 1; i >= 0; i--)
                {
                    ...
                    if (i > spline.allSplineObjects.Count - 1)
                        continue;
        ...
        public static void Update(Spline spline)
        {
            foreach(SplineObject so in spline.allSplineObjects)
            {
                ...
                if (so.monitor.StaticChange(true))
                {
        ...
        public static bool CanBeDeformed(SplineObject so)
        {
            ...
            foreach(MeshContainer mc in so.meshContainers)
            {
                ...
                if (originMesh == null)
                {
                    if (!noOriginMeshWarningContainer.Contains(so) && PositionTool.activePart != PositionTool.Part.NONE)
                    {
