...
namespace SplineArchitect
{
    public static class EHandleDeformation
    {
        ...
        {
            HandleDeformation.GetActiveWorkers(DeformationWorker.Type.EDITOR, activeWorkersList);

            if (activeWorkersList.Count == 0)
                return;

            for (int i = 0; i < activeWorkersList.Count; i++)
            {
        ...
        public static void TryDeform(Spline spline, bool deformLinks = true)
        {
            ...
            bool segmentChange = spline.monitor.SegmentChange();
            bool normalTypeChange = spline.monitor.NormalTypeChange();
            bool resolutionChange = spline.monitor.ResolutionChange();
            bool splineExtraChange = spline.monitor.ExtraChange();
            bool noiseChange = spline.monitor.NoiseChange();
            bool splineChange = isEditorDirty || segmentChange || normalTypeChange || resolutionChange || splineExtraChange || noiseChange;
            ...
            for (int i = 0; i < spline.allSplineObjects.Count; i++)
            {
                ...
                bool soChange = combinedParentPosRotScaleChange || posRotSplineSpaceChange || mirrorChange || extraChange || scaleChange || normalsChange || splineLengthChange;
                ...
                if(normalsChange)
                {
                    if (so.normalType == SplineObject.NormalType.DO_NOT_CALCULATE)
                    {
                        so.SetOriginNormalsOnAll();
                        so.SetOriginTrianglesOnAll();
                        so.SetOriginTangentsOnAll();
                    }
                    else if (so.normalType == SplineObject.NormalType.UNITY_CALCULATED_SEAMLESS)
            ...
            HandleDeformation.UpdateFollowers(spline);
