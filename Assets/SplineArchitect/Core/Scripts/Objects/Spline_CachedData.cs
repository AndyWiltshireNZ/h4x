...
namespace SplineArchitect.Objects
{
    public partial class Spline : MonoBehaviour
    {
        ...
        public void UpdateCachedData()
        {
            ...
            {
                CalculateSplineData();
                if (normalType == NormalType.DYNAMIC) CalculateCachedNormals();
            ...
            void ValidateShape()
            {
                ...
                {
                    Vector3 anchor = segments[0].GetPosition(Segment.ControlHandle.ANCHOR, Space.Self);
                    Vector3 tangentB = segments[0].GetPosition(Segment.ControlHandle.TANGENT_B, Space.Self);

                    if (GeneralUtility.IsEqual(anchor, tangentB))
                        isInvalidShape = true;

                    anchor = segments[segments.Count - 1].GetPosition(Segment.ControlHandle.ANCHOR, Space.Self);
                    Vector3 tangentA = segments[segments.Count - 1].GetPosition(Segment.ControlHandle.TANGENT_A, Space.Self);
