...
namespace SplineArchitect.Objects
{
    ...
    public partial class SplineObject : MonoBehaviour
    {
        ...
        [HideInInspector]
        public MeshMode meshMode = MeshMode.SAVE_IN_BUILD;
        ...
        //Runtime data static
        [NonSerialized]
        public static List<Vector3> normalsContainer = new List<Vector3>();
        [NonSerialized]
        public static List<Vector4> tangentsContainer = new List<Vector4>();
        [NonSerialized]
        public static List<int> trianglesContainer = new List<int>();
        ...
        public void Initalize()
        {
            ...
            {
#if UNITY_EDITOR
                //We need to check fi the prefab stage is open. Or we will get this error message after applying data to a prefab, but we should not get this error in that case.
                if(!EHandlePrefab.prefabStageOpen)
        ...
        public void ConvertToDeformation()
        {
            ...
            HandleDeformation.Deform(this, DeformationWorker.Type.RUNETIME, splineParent);
        }
        ...
        public void ReorderForParentHierarchy()
        {
            ...
            for (int i = 0; i < splineParent.allSplineObjects.Count; i++)
            {
                if (splineParent.allSplineObjects[i] != soParent)
                    continue;

                //Always add child directly after parent in list
                if (i + 1 >= splineParent.allSplineObjects.Count)
                    splineParent.AddSplineObject(this);
                else
        ...
        public SnapData CalculateSnapData()
        {
            ...
            float GetClosestPoint(SnapMode snapMode, float point, out float distance)
            {
                ...
                {
                    foreach (SplineObject so in splineParent.allSplineObjects)
                    {
                        if (so == this)
                            continue;
        ...
        public void SetSeamlessTrianglesOnAll()
        {
            foreach (MeshContainer mc in meshContainers)
            {
